...
if __name__ == "__main__":
    ...
    # --- Étape A : On prépare l'action (Créer un doc) ---
    def creer_doc():
        return api_request(
            "POST", 
            f"{BASE}/documents", 
            data={"title": "Test Résilience", "content": "Ceci est un test"},
            token=TOKEN
        )

    # --- Étape B : On lance l'action AVEC la protection ---
    print("Lancement de la requête avec protection contre les pannes...")
    status, body = request_with_retry(creer_doc)

    if status == 201:
        print("✅ Réussite finale !")
    else:
        print(f"❌ Échec définitif après plusieurs essais (Code: {status})")